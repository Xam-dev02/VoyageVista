...
if ($method === 'GET' && $id === null && $seg0 === 'panier') {
    ...
    if (!$row) {
        // Vérifier que l'utilisateur existe encore (sécurité post-réimport BDD)
        $chk = db()->prepare('SELECT id_utilisateur FROM utilisateur WHERE id_utilisateur = ?');
        $chk->execute([$userId]);
        if (!$chk->fetch()) {
...
if ($method === 'POST' && $id === null && $seg0 === 'etapes' && $seg1 !== null && $seg2 === 'activites') {
    ...
    // Récupérer l'activité et vérifier les places disponibles
    $stmtAct = db()->prepare('SELECT prix_personne, places_disponibles, statut, nom FROM activite WHERE id_activite = ?');
    $stmtAct->execute([$idActivite]);
    $actRow = $stmtAct->fetch(PDO::FETCH_ASSOC);
    if (!$actRow) {
        Response::notFound('Activité introuvable.');
    }
    if ($actRow['statut'] === 'complet' || (int)$actRow['places_disponibles'] === 0) {
        Response::error('L\'activité « ' . $actRow['nom'] . ' » est complète.', 422);
    }
    if ((int)$actRow['places_disponibles'] < $nbPersonnes) {
        Response::error('Seulement ' . $actRow['places_disponibles'] . ' place(s) disponible(s) pour « ' . $actRow['nom'] . ' ».', 422);
    }
    ...
    // Notification : activité ajoutée
    $notifMsg = "L'activité « {$actRow['nom']} » ({$nbPersonnes} pers.) a été ajoutée à votre itinéraire.";
    db()->prepare('INSERT INTO notification (titre, message, type, id_utilisateur, id_voyage) VALUES (?, ?, \'rappel\', ?, ?)')
        ->execute(['Activité réservée', $notifMsg, $userId, $idVoyage]);
...
if ($method === 'PUT' && $id === null && $seg0 === 'etapes' && $seg1 !== null) {
    ...
    if (!empty($fields)) {
        ...
        // Notifications selon ce qui a été modifié
        if (array_key_exists('id_transport', $data) && $data['id_transport'] !== null) {
            $t = db()->prepare('SELECT compagnie FROM transport WHERE id_transport = ?');
            $t->execute([(int)$data['id_transport']]);
            $tRow = $t->fetch();
            if ($tRow) {
                db()->prepare('INSERT INTO notification (titre, message, type, id_utilisateur, id_voyage) VALUES (?, ?, \'rappel\', ?, ?)')
                    ->execute(['Transport ajouté', 'Le transport ' . $tRow['compagnie'] . ' a été ajouté à votre itinéraire.', $userId, $idVoyage]);
            }
        }
        if (array_key_exists('id_hebergement', $data) && $data['id_hebergement'] !== null) {
            $h = db()->prepare('SELECT nom FROM hebergement WHERE id_hebergement = ?');
            $h->execute([(int)$data['id_hebergement']]);
            $hRow = $h->fetch();
            if ($hRow) {
                db()->prepare('INSERT INTO notification (titre, message, type, id_utilisateur, id_voyage) VALUES (?, ?, \'modification\', ?, ?)')
                    ->execute(['Hébergement ajouté', 'L\'hébergement « ' . $hRow['nom'] . ' » a été ajouté à votre itinéraire.', $userId, $idVoyage]);
            }
        }
    }
