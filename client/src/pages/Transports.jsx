...
export default function Transports() {
  ...
  // Pré-remplissage depuis les params URL directs (bouton "Ajouter transport" de l'itinéraire)
  useEffect(() => {
    if (autoDone) return;
    const paramArrivee = searchParams.get('id_arrivee');
    const paramOrigine = searchParams.get('id_origine');
    const paramEtape   = searchParams.get('etape');

    if (paramArrivee) {
      // Vient de l'itinéraire : arrivée connue, départ à choisir (ou pré-rempli)
      setIdArrivee(paramArrivee);
      if (paramOrigine) setIdOrigine(paramOrigine);
      if (paramEtape)   setCurrentEtapeId(Number(paramEtape));
      // Si les deux villes sont connues, on peut lancer la recherche
      if (paramOrigine) doSearch(paramOrigine, paramArrivee, '', 'prix');
      setAutoDone(true);
      return;
    }

    // Fallback : premier tronçon sans transport depuis l'itinéraire chargé
    if (legs.length === 0) return;
    const wanted = paramEtape;
  ...
  async function handleSelect(t) {
    ...
    try {
      // Date d'arrivée à la destination = date du billet (partie date seulement)
      const dateArr = t.date_arrivee ? t.date_arrivee.substring(0, 10) : null;

      if (currentEtapeId) {
        await api.put(`/voyages/etapes/${currentEtapeId}`, {
          id_transport: t.id_transport,
        ...
        if (etape) {
          await api.put(`/voyages/etapes/${etape.id_etape}`, {
            id_transport: t.id_transport,
            date_arrivee: dateArr,
          });
        } else {
          ...
          if (ne) await api.put(`/voyages/etapes/${ne.id_etape}`, {
            id_transport: t.id_transport,
            date_arrivee: dateArr,
          });
        }
      ...
      <div className="container section">
        <div className="transports-layout">
          <div className="card-dark transports-card">
            ...
            {searchParams.get('id_arrivee') && (() => {
              const nomArr = destinations.find(d => String(d.id_destination) === searchParams.get('id_arrivee'))?.nom;
              const dateDepart = searchParams.get('date_depart');
              return nomArr ? (
                <div className="transports-context">
                  <span>Transport vers <strong>{nomArr}</strong>{dateDepart ? ` · aux alentours du ${dateDepart}` : ''}</span>
