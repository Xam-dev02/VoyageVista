...
export default function Profil() {
  ...
  const [editResa, setEditResa] = useState(null); // réservation en cours de modification
  ...
  async function sauverModifResa(methode) {
    try {
      await api.put(`/reservations/${editResa.id_reservation}`, { methode_paiement: methode });
      setToast('Réservation modifiée.');
      setEditResa(null);
      reload();
    } catch (e) { setToast(e.message || 'Erreur.'); }
  }
    ...
    <div className="profil-page page">
      <div className="container section">
        <div className="flex-between wrap gap-16 mb-8">
          ...
          <button className="btn btn--white profil-logout" onClick={handleLogout}>Se déconnecter</button>
        </div>
              ...
              {reservations.map(r => (
                <div key={r.id_reservation} className="profil-row">
                  ...
                  <div className="profil-row-actions">
                    ...
                    {r.statut !== 'annulee' && (
                      <>
                        <button className="pill pill--sm" onClick={() => setEditResa(r)}>Modifier</button>
                        <button className="pill pill--sm pill--ghost profil-del"
      ...
      {/* Modal modification de réservation */}
      {editResa && (
        <div className="profil-modal-overlay" onClick={() => setEditResa(null)}>
          <div className="profil-modal" onClick={e => e.stopPropagation()}>
            <h3 className="mb-16">Modifier la réservation {editResa.reference}</h3>
            <p className="muted mb-16">Montant : {euros(editResa.montant_total)}</p>
            <div className="field">
              <label>Méthode de paiement</label>
              <select className="select" defaultValue={editResa.methode_paiement || 'carte'}
                onChange={e => setEditResa({ ...editResa, _methode: e.target.value })}>
            ...
            <div className="flex gap-8 mt-16">
              <button className="btn btn--dark" onClick={() => sauverModifResa(editResa._methode || editResa.methode_paiement || 'carte')}>
                Enregistrer
              </button>
              <button className="btn btn--outline" onClick={() => setEditResa(null)}>Annuler</button>
            </div>
          </div>
        </div>
      )}
